...
/** Cria templates de área (circle/cone/ray): clique fixa a origem, arrasta dimensiona/direciona. */
...
class TemplateIdle extends Tool {
  ...
  override onPointerMove(info: CanvasPointerInfo): void {
    drawTemplatePreview(this, info.point, this.options.template.distance, 0);
  }
...
class TemplatePlacing extends Tool {
  ...
  override onEnter(info?: unknown): void {
    ...
    this.distance = this.options.template.distance;
    this.direction = 0;
  }
  ...
  override onPointerUp(): void {
    const opts = this.options.template;
    void this.canvas.templates.create({
      shape: opts.shape,
      x: this.origin.x,
      y: this.origin.y,
...
function drawTemplatePreview(tool: Tool, origin: Point, distanceCells: number, direction: number): void {
  const opts = tool.options.template;
