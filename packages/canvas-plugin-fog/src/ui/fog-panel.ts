...
export class OpenVTTFogPanel extends HTMLElement {
  ...
  private bind(): void {
    ...
    const dyn = dynamicBus(this._canvas.bus);
    this.unsubs.push(
      dyn.on('fog:change', () => {
  ...
  private render(): void {
    ...
    const fog = this._canvas?.layers.getLayer<FogOfWarLayer>('fog');
    const tool = this._canvas?.getCurrentToolId() ?? 'select';
    const enabled = fog?.enabled ?? false;
    const brushSize = Number(this._canvas?.tools?.options.fogReveal?.brushSize ?? 100);
    this.shadowRoot.innerHTML = `
      ...
      <div class="panel" part="panel">
        ...
        <div class="row">
          ...
          <input type="range" class="brush" min="20" max="300" step="10" value="${brushSize}" ${enabled ? '' : 'disabled'} />
        </div>
        <div class="row">
  ...
  private wire(): void {
    ...
    $('.brush').addEventListener('input', (e) => {
      const options = canvas.tools?.options;
      if (!options) return;
      const value = Number((e.target as HTMLInputElement).value);
      options.fogReveal = { ...options.fogReveal, brushSize: value };
      options.fogPaint = { ...options.fogPaint, brushSize: value };
    });
    ...
    $('.apply-selected').addEventListener('click', () => {
      for (const obj of canvas.selected) {
        if (obj.objectType === 'token') canvas.documents.update('token', obj.id, { visionRadius: this.visionRadius });
      }
    });
    $('.apply-all').addEventListener('click', () => {
      const layer = canvas.documents.layer('token');
      if (!layer) return;
      for (const token of layer.placeables) {
        canvas.documents.update('token', token.id, { visionRadius: this.visionRadius });
      }
    });
