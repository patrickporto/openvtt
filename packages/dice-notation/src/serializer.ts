...
function serialize(expr: RollExpr): Out {
  ...
  switch (key) {
    ...
    case 'max':
    case 'clamp':
      return { text: `${key}(${(value as RollExpr[]).map(serialize).map((o) => o.text).join(', ')})`, prec: ATOM };
    default:
...
function serializeModifier(mod: Modifier): string {
  ...
  if (op === 'keep-highest' || op === 'keep-lowest' || op === 'drop-highest' || op === 'drop-lowest') {
    return mod.count != null ? `${op}${mod.count}` : op;
  }
  if (op === 'min' || op === 'max') return `${op}${mod.value}`;
  if (op === 'margin-success') return `${op}${mod.target}`;
  if (op === 'reroll-once' || op === 'reroll-recursive') {
    return mod.compare ? `${op}${serializeComparison(mod.compare)}` : op;
  }
  if (op === 'explode' || op === 'explode-once' || op === 'explode-compound' || op === 'explode-penetrating') {
    const cap = mod.cap != null ? String(mod.cap) : '';
    const cmp = mod.compare ? serializeComparison(mod.compare) : '';
    return `${op}${cap}${cmp}`;
  }
...
function serializeComparison(compare: Comparison): string {
